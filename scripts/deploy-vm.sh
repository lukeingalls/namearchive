...
gcloud compute ssh "${INSTANCE_NAME}" \
  --project "${PROJECT_ID}" \
  --zone "${ZONE}" \
  --command "sudo mkdir -p /opt/namearchive/releases /opt/namearchive/shared/data /opt/namearchive/shared/og-cache && \
    sudo mkdir -p '${REMOTE_RELEASE}' && \
    sudo tar -xzf '${REMOTE_TMP}' -C '${REMOTE_RELEASE}' && \
    sudo rm -rf '${REMOTE_RELEASE}/apps/server/data' '${REMOTE_RELEASE}/apps/server/.og-cache' && \
    sudo ln -sfn /opt/namearchive/shared/data '${REMOTE_RELEASE}/apps/server/data' && \
