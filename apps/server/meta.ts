import { babyNamesDatabase } from '../client/src/app/data/babyNamesData';
...
function parseNameFromPath(pathname: string): string | null {
...
export function getPageMeta(pathname: string, origin: string): PageMeta {
  const rawName = parseNameFromPath(pathname);

  if (rawName) {
    const canonicalName = resolveName(rawName);
    if (canonicalName && babyNamesDatabase[canonicalName]) {
      return {
  ...
  if (pathname === '/about') {
    return {
      ...
      description: 'Learn about name archive and its mission to make naming history easy to explore.',
      ogImage: null,
    };
  }
  ...
  return {
    title: 'name archive',
    description: 'Explore historical U.S. baby naming trends from 1900 to 2026 on namearchive.org.',
    ogImage: null,
  };
}
...
export function renderHeadTags(pathname: string, origin: string, meta: PageMeta): string {
